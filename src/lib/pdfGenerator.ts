...
// Generate VAT Return HTML
function generateVatReturnHTML(reportData: any, tenantName: string, tin: string, period: string): string {
  const data = reportData;
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .org-header { display: flex; justify-content: center; gap: 30px; font-weight: bold; margin-bottom: 15px; }
    .vat-table { width: 100%; border: 1px solid #000; border-collapse: collapse; }
    .vat-table th { background: #f0f0f0; border: 1px solid #000; padding: 8px; text-align: left; text-transform: uppercase; font-size: 8pt; }
    .vat-table td { border: 1px solid #000; padding: 10px; }
    .box-num { font-weight: bold; width: 60px; display: inline-block; }
    .amount { text-align: right; font-weight: bold; font-family: 'Courier New', monospace; }
    .footer-section { margin-top: 50px; display: flex; justify-content: space-between; font-size: 9pt; font-weight: bold; text-transform: uppercase; }
    .sig-box { border-top: 1px solid #000; width: 220px; text-align: center; padding-top: 5px; }
  </style>
</head>
<body>
  <div class="header">
    <h1>Value Added Tax (VAT) Return</h1>
    <p>Federal Democratic Republic of Ethiopia Ministry of Revenue</p>
  </div>
  ...
  <table class="vat-table">
    <thead>
      <tr>
        <th>Description of Taxable Activity</th>
        <th style="text-align: center;">Base Amount (ETB)</th>
        <th style="text-align: center;">VAT Amount (15%)</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><span class="box-num">[Box 5]</span> Standard Taxable Sales & Services</td>
        <td class="amount">${formatCurrency(data.box5TaxableSales)}</td>
        <td class="amount">${formatCurrency(data.box5Vat)}</td>
      </tr>
      <tr>
        <td><span class="box-num">[Box 10]</span> Standard Taxable Purchases (Local & Import)</td>
        <td class="amount">${formatCurrency(data.box10TaxablePurchases)}</td>
        <td class="amount">(${formatCurrency(data.box10Vat)})</td>
      </tr>
      <tr style="background: #f9f9f9; font-size: 11pt;">
        <td style="font-weight: 900; text-transform: uppercase;"><span class="box-num">[Box 15]</span> Net-credit (or Payable) to the Filer</td>
        <td></td>
        <td class="amount" style="text-decoration: underline double;">${formatCurrency(data.box15NetCredit)}</td>
      </tr>
    </tbody>
  </table>

  <div class="footer-section">
    <div class="sig-box">Accountant Name / Stamp</div>
    <div class="sig-box">Tax Authority Received Date</div>
  </div>

  <div style="margin-top: 40px; font-size: 8pt; color: #666; text-align: center;">
    Generated by Hisabe Accounting System | Strictly Verified & Reconciled
  </div>
</body>
</html>
  `;
...
export async function generateReportPDF(
  ...
  switch (report.reportType) {
    ...
    case 'vat_declaration':
      // report.data for vat_declaration usually follows the structure from accountingEngine
      // but let's ensure we use organizationName and tin from tenant if not present
      html = generateVatReturnHTML(
        report.data,
        tenant.organizationName || tenant.name,
        tenant.registration?.tin || (tenant as any).tin,
        `${formatDate(new Date(report.period.startDate))} to ${formatDate(new Date(report.period.endDate))}`
      );
      break;
    default:
